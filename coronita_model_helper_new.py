...
def seir_model_cohort(start_dt, model_dict, exposed_0=100, infectious_0=100):
    ...
    next_susceptible_vaxxed = vax_new_people_daily.loc[start_dt]
    ...
    for t_ in t[:-1]:
        ...
        if cohort_strt == last_obs_rt:
            ...
            r_t_preimmune = pd.Series()
            # print(f'{cohort_strt} next_suspop: {next_suspop}')
        elif (cohort_strt > last_obs_rt):
            r_t.loc[cohort_strt] = r_t.loc[cohort_strt] * (next_suspop / suspop_lastdayofobs)
        ...
        prop_recovered = next_recovered / model_dict['tot_pop']
...
def make_model_dict_state(state_code, abbrev_us_state, df_census, df_st_testing_fmt, df_hhs_hosp, df_can, df_counties, df_vax_hes,
    ...
    model_dict['covid_params']['est_vax_hes_strong_pop_18plus'] = df_vax_hes_state.loc[state_code, 'est_vax_hes_pop_18plus']
...
def make_model_dict_us(df_census, df_st_testing_fmt, df_hhs_hosp, df_can, df_counties, covid_params, d_to_forecast = 75,
                        df_mvmt=pd.DataFrame(), df_interventions=pd.DataFrame()):
    model_dict = {}

    model_dict['region_code'] = 'US'
    model_dict['region_name'] = 'United States'
    model_dict['tot_pop'] = df_census.loc[(df_census.SUMLEV == 40), 'pop2019']
